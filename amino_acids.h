...
#define NUM_CODONS (ALPHABET_SIZE*ALPHABET_SIZE*ALPHABET_SIZE) //+1)
#define NUM_CODONS_AND_INDEL (ALPHABET_SIZE*ALPHABET_SIZE*ALPHABET_SIZE+1)
...
#define is_gap(x) (dnachar_to_code[(int)(x)] == 5)

#define is_codon_gap(codon) (base_to_code((codon)[0]) == 5 && \
					   base_to_code((codon)[1]) == 5 && \
					   base_to_code((codon)[2]) == 5)
#define is_not_codon(codon) !is_codon_gap(codon) && \
						(base_to_code((codon)[0]) == 5 || \
						 base_to_code((codon)[1]) == 5 || \
						 base_to_code((codon)[2]) == 5)

extern const char dnachar_to_code[128];

extern const char rna_to_codon[NUM_CODONS_AND_INDEL];

extern const char codon_to_dna[NUM_CODONS_AND_INDEL][4];
...
#define amino_to_code(codon) \
		is_not_codon(codon) ? 65 : \
			(is_codon_gap(codon) ? 64 : \
...
void map_partial_gaps_to_consensus(char **seqs, int num_seqs, int num_sites,
                           const char *consensus);
