...
const char dnachar_to_code[128] = {  4,  4,  4,  4,  4,  4,  4,  4,  4,  4,  4,  4,  4,  4,  4,  4,
                                     ...
                                     4,  2,  4,  1,  4,  4,  4,  3,  4,  4,  4,  4,  4,  4,  4,  4,
                                     4,  4,  4,  4,  0,  4,  4,  4,  4,  4,  4,  4,  4,  4,  4,  4};
// Above is the ASCII character set, where we consider only those values that are 
// T,t, C,c, A,a, G,g in our alphabet.
...
// ttt ... ggg.
const char rna_to_codon[NUM_CODONS_AND_INDEL] =
{'F','F','L','L','S','S','S','S',
 'Y','Y','*','*','C','C','*','W',
 'L','L','L','L','P','P','P','P',
 'H','H','Q','Q','R','R','R','R',
...
const char codon_to_dna[NUM_CODONS_AND_INDEL][4] =
                           ...
                           const char *consensus)
{
  ...
  for(i = 0; i < len; i++) {
    for(s = 0; s < num_seqs; s++) {
      if(!is_base(seqs[s][i]) || is_gap(seqs[s][i]))
                                   ...
                                   const char *consensus)
{
  ...
  for(i = 0; i < num_sites; i++) {
    for(s = 0; s < num_seqs; s++) {
      if(is_not_codon(seqs[s]+i*3)) {
        if(!is_base(seqs[s][i*3]) || is_gap(seqs[s][i*3])) {
          seqs[s][i*3] = consensus[i*3];
        }
        if(!is_base(seqs[s][i*3+1]) || is_gap(seqs[s][i*3+1])) {
          seqs[s][i*3+1] = consensus[i*3+1];
        }
        if(!is_base(seqs[s][i*3+2]) || is_gap(seqs[s][i*3+2])) {
          seqs[s][i*3+2] = consensus[i*3+2];
        }
      }
    }
  }
